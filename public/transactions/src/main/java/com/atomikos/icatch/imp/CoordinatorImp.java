        ...
        FSMPreEnterListener, FSMTransitionListener
{
    ...
    boolean requiresHeuristics() {
        boolean ret = true;
        String recoveryCoordinatorURI = superiorCoordinatorId();
        boolean remoteRecoveryEnabled = false;
        try {
            Configuration.getConfigProperties().getProperty(ConfigProperties.RECOVERY_COORDINATOR_BASE_URI);
            remoteRecoveryEnabled = true;
        } catch (IllegalArgumentException notSet) {
        }
        if (recoveryCoordinatorURI != null && recoveryCoordinatorURI.startsWith("http") && remoteRecoveryEnabled) {
            ret = false;
        }
        return ret;
    }
