...
class ActiveStateHandler extends CoordinatorStateHandler
{
    ...
    HeurMixedException, SysException
  {
    ...
    List<Participant> participants = getCoordinator().getParticipants();
    ...
    try
    {
      ...
      {
        PrepareMessage pm = new PrepareMessage(participant, result);
        if (getCascadeList() != null && participant.getURI() != null)
        { //null for OTS
          Integer sibnum = (Integer) getCascadeList().get(participant.getURI());
          if (sibnum != null)
          { // null for local participant!
