	...
	public Object invoke ( Object proxy, Method method, Object[] args ) throws JMSException
	{
		...
		synchronized (this) {
			...
			if ( SESSION_TRANSACTION_METHODS.contains ( methodName ) ) {
				...
				if ( LOGGER.isInfoEnabled() ) Configuration.logInfo ( this + ": " + msg );
			...
			if ( CLOSE_METHOD.equals ( methodName ) ) {
				...
				if ( LOGGER.isInfoEnabled() ) Configuration.logInfo ( this + ": closing session " + this + " - is terminated ? " + state.isTerminated() );
			...
			if (PRODUCER_CONSUMER_METHODS.contains(methodName)) {
				if ( LOGGER.isInfoEnabled() ) Configuration.logInfo ( this + ": calling " + methodName + " on JMS driver session " + delegate );
			...
			try {
				if ( LOGGER.isInfoEnabled() ) Configuration.logInfo ( this + ": calling " + methodName + " on JMS driver session..." );
