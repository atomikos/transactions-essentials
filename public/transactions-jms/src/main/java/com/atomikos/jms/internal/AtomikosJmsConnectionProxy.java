...
public class AtomikosJmsConnectionProxy extends DynamicProxySupport<XAConnection>
    ...
    public AtomikosJmsConnectionProxy(XAConnection delegate, int sessionCreationMode,
    ...
    public Session createSession(boolean transacted, int acknowledgeMode) throws JMSException {
        ...
        synchronized (sessions) { //cf case 123502: deadlock
            ...
            if (createXaSession(transacted)) {
                ...
                if (jtaTransactionExistsForThread()) {
                    if (LOGGER.isDebugEnabled()) {
    ...
    private boolean jtaTransactionExistsForThread() {
        ...
        return ct != null && TransactionManagerImp.isJtaTransaction(ct);
    }
    ...
    private boolean createXaSession(boolean sessionTransactedFlag) throws JMSException {
    	if (sessionCreationMode == SessionCreationMode.JMS_2_0) {
    		if (jtaTransactionExistsForThread()) { 
    			return true; // cf case 185528
    		} else {
    			return !props.getLocalTransactionMode();
    		}
    	} else if (sessionCreationMode == SessionCreationMode.PRE_6_0) {
            return !props.getLocalTransactionMode();
        } else if (sessionCreationMode == SessionCreationMode.PRE_3_9) {
            return sessionTransactedFlag && !props.getLocalTransactionMode();
        } else {
        	AtomikosJMSException.throwAtomikosJMSException("Unhandled value of SessionCreationMode: " + sessionCreationMode);
        }
    	return true; // we should never get here - just keep compiler happy
    }
    ...
    public static Connection newInstance(int sessionCreationMode, XAConnection xaConnection,
        ...
        AtomikosJmsConnectionProxy proxy = new AtomikosJmsConnectionProxy(xaConnection, sessionCreationMode,
