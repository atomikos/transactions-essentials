...
class AtomikosNonXAPooledConnection extends AbstractXPooledConnection<Connection>
{
	...
	protected Connection doCreateConnectionProxy() throws CreateConnectionException 
	{
		...
		if ( canBeRecycledForCallingThread() ) {
			...
			//AtomikosThreadLocalConnection previous = (AtomikosThreadLocalConnection) Proxy.getInvocationHandler(ret);
			...
			ret = ( Connection ) AtomikosThreadLocalConnection.newInstance ( this , props.getUniqueResourceName() );
		}
		return ret;
	}
	...
	public boolean isAvailable() {
		...
		if ( handle != null ) {
			AtomikosThreadLocalConnection previous = (AtomikosThreadLocalConnection) Proxy.getInvocationHandler(handle);
