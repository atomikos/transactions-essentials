...
public class AtomikosNonXADataSourceBean extends AbstractDataSourceBean 
{
	 ...
	 * 
	 * Setting this to true will avoid warnings/errors on 2-phase commit. ReadOnly mode
	...
	public synchronized Connection getConnection() throws SQLException
	{
		...
		if (Proxy.getInvocationHandler(connection) instanceof JtaAwareThreadLocalConnection) { // not so for local transactions		    
		    JtaAwareThreadLocalConnection previous = (JtaAwareThreadLocalConnection) Proxy.getInvocationHandler(connection);
	...
	public void setLocalTransactionMode(boolean value) {
	    this.ignoreJtaTransactions = value;
	}
