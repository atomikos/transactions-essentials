...
public class AtomikosNonXAPooledConnection extends AbstractXPooledConnection<Connection>
{
	...
	{
	    this(wrapped, props, readOnly, false);
	}
	
	public AtomikosNonXAPooledConnection (Connection wrapped, ConnectionPoolProperties props, boolean readOnly, boolean ignoreJtaTransactions) {
	       super ( props );
	...
	protected Connection doCreateConnectionProxy() throws CreateConnectionException 
	{
		...
		if ( canBeRecycledForCallingThread() ) {
			...
			AtomikosJdbcThreadLocalConnection proxy = new AtomikosJdbcThreadLocalConnection( this , props.getUniqueResourceName(), ignoreJtaTransactions);
	...
	public boolean canBeRecycledForCallingThread() 
	{
		boolean ret = false;
		Object handle = getCurrentConnectionProxy();
		if ( handle != null && !ignoreJtaTransactions) {
