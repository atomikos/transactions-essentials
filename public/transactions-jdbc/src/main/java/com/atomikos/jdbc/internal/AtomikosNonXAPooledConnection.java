...
public class AtomikosNonXAPooledConnection extends AbstractXPooledConnection<Connection>
{
	...
	protected Connection doCreateConnectionProxy() throws CreateConnectionException 
	{
		...
		if ( canBeRecycledForCallingThread() ) {
			...
			if (props.getLocalTransactionMode()) {			    
			    JtaUnawareThreadLocalConnection proxy = new JtaUnawareThreadLocalConnection(this);
			    ret = proxy.createDynamicProxy();
			} else {
			    JtaAwareThreadLocalConnection proxy = new JtaAwareThreadLocalConnection( this , props.getUniqueResourceName());
			    ret = proxy.createDynamicProxy();
			}
		}
	...
	public boolean canBeRecycledForCallingThread() 
	{
		boolean ret = false;
		Object handle = getCurrentConnectionProxy();
		if ( handle != null && !props.getLocalTransactionMode()) {
			 CompositeTransactionManager ctm = Configuration.getCompositeTransactionManager ();
			 CompositeTransaction ct = null;
			 if ( ctm != null ) ct = ctm.getCompositeTransaction ();
			 if ( ct != null && TransactionManagerImp.isJtaTransaction(ct) ) {
				 JtaAwareThreadLocalConnection previous = (JtaAwareThreadLocalConnection) Proxy.getInvocationHandler(handle);
