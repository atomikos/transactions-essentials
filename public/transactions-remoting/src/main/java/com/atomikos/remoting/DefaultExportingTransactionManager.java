...
public class DefaultExportingTransactionManager implements ExportingTransactionManager {
	...
	public Propagation getPropagation() throws SysException, IllegalStateException {
		...
		try {
		    recoveryCoordinatorURI = Configuration.getConfigProperties().getProperty(ConfigProperties.RECOVERY_COORDINATOR_BASE_URI);
		    recoveryCoordinatorURI = recoveryCoordinatorURI + "/" + ct.getCompositeCoordinator().getCoordinatorId();
