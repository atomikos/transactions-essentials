...
public class DefaultImportingTransactionManager implements ImportingTransactionManager {
	...
	public Extent terminated(boolean commit) throws SysException, RollbackException {
		...
		ret = new Extent(extent); // cf case 185558: create extent without any direct participants
		ret.add(new ParticipantAdapter(participantURI), 1);
		LOGGER.logDebug("Returning extent: " + ret);
		return ret;
	}
