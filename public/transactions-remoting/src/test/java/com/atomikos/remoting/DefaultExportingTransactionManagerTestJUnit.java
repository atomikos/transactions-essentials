...
public class DefaultExportingTransactionManagerTestJUnit {
	...
	private String givenTid = "4567";
	private String givenParentTid = "98";
	...
	@Test
	public void testPropgationRecoveryCoordinatorURI() {
		givenExistingTransaction();
		Propagation p = sut.getPropagation();
		assertEquals(givenCoordinatorId, p.getRecoveryCoordinatorURI());
	}
	...
	private CompositeTransaction givenExistingTransaction() {
		...
		Mockito.when(compositeCoordinator.getCoordinatorId()).thenReturn(givenCoordinatorId);
		...
		Mockito.when(mockedCT.getTid()).thenReturn(givenTid);
