...
import com.atomikos.recovery.tcc.rest.InMemoryParticipantRepository;
...
public class CoordinatorImp implements Coordinator {
	...
	public CoordinatorImp() {
		repository = InMemoryParticipantRepository.INSTANCE;
	}
	...
	public void confirm(Transaction transaction) {
		...
		saveTaasParticipantsForRecovery(taasTransaction.getTid(), transaction);
		commitTransaction(taasTransaction);
	}

	private void saveTaasParticipantsForRecovery(String coordinatorId, Transaction transaction) {
		Set<String> uris = new HashSet<>();
		for (ParticipantLink link : transaction.getParticipantLinks()) {
			uris.add(link.getUri());
		}
		repository.save(coordinatorId, uris);
	}
