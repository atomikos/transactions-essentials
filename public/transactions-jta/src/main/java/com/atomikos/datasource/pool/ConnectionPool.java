...
public abstract class ConnectionPool<ConnectionType> implements XPooledConnectionEventListener<ConnectionType>
{
	...
	private XPooledConnection<ConnectionType> createPooledConnection()
		...
		EventPublisher.publish(new PooledConnectionCreatedEvent(properties.getUniqueResourceName()));
		return xpc;
	}
	...
	protected void destroyPooledConnection(XPooledConnection<ConnectionType> xpc, boolean reap) {
		xpc.destroy(reap);
		EventPublisher.publish(new PooledConnectionDestroyedEvent(properties.getUniqueResourceName()));
	}
	...
	public synchronized void reapPool()
	{
		...
		while ( it.hasNext() ) {
			...
			if ( inUse && ( ( now - maxInUseTime * 1000 ) > lastTimeReleased ) ) {
				...
				xpc.destroy(true);
				it.remove();
				EventPublisher.publish(new PooledConnectionReapedEvent(properties.getUniqueResourceName()));
			}
		}
		logCurrentPoolSize();
	}
