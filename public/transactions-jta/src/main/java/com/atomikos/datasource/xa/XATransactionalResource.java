...
public abstract class XATransactionalResource implements TransactionalResource
{
            ...
            throws ResourceException
    {
        ...
        try {
            return xatxres.getXAResource().isSameRM(this.getXAResource()); //cf case 170618
        } catch (XAException e) {
    ...
    public boolean recover(long startOfRecoveryScan) {
    	...
    	if (xaResourceRecoveryManager != null) { //null for LogCloud recovery
    		...
    		try {
    			ret = xaResourceRecoveryManager.recover(getXAResource(), startOfRecoveryScan, uniqueResourceName);	
			} catch (Exception e) {
